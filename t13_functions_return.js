/****************************
Name of task:
****************************/

console.log("Running t13_functions_return.js")
console.log("/*This is annoying*/")

//Varibles
let userMoney;
let userAge;
let userName;
let currentYear;
let plusThree;
let divideTwo;

userName = "Lukys";
userAge = 15;
userMoney = 100;
currentYear = 2026;
divideTwo = userMoney / 2;
plusThree = userMoney / 2 + 3;


const OUTPUT = document.getElementById("spaceForJavaScriptOutput");
/****************************
Main code:
****************************/

function start() {

  welcome()

  OUTPUT.innerHTML += "<p>Hi " + userName;
  OUTPUT.innerHTML += "<p>As of " + currentYear + " you are " + userAge + " years old";
  let yearBorn = currentYear - userAge;
  OUTPUT.innerHTML += "<p>You were born in " + yearBorn;
  OUTPUT.innerHTML += "<p>You have " + userMoney + " dollars";
  OUTPUT.innerHTML += "<p>You spend half of your money, now you have " + divideTwo;
  OUTPUT.innerHTML += "<p>Then you get $3, now you have " + plusThree;
}


/****************************
Functions:
****************************/

function welcome() {

  OUTPUT.innerHTML += "<p>Welcome to the shop";
}

displayProduct("Chips", 3);
displayProduct("Chocolate bar", 4);
displayProduct("Drink", 2.50);

function displayProduct(_name, _price) {
  OUTPUT.innerHTML += "<p> " + _name + ": " + _price;
}


function getFormInput() {
  const MONEY_FIELD = document.getElementById("moneyField");
  let userMoney = Number(MONEY_FIELD.value);
  OUTPUT.innerHTML = "<p>You have " + userMoney + ".</p>";

  if (userMoney >= 4) {
    OUTPUT.innerHTML += "<p>You CAN afford a chocolate bar";
  } else {
    OUTPUT.innerHTML += "<p>Sorry you CAN'T afford a chocolate bar";
  }
}


calculateChange("Money", Number(MONEY_FIELD.value));
calculateChange("Price", 4);

let change = calculateChange(_money, _price);
let calculateChange = _money - _price;

console.log("you gave me "+change+" dollars. It should be 3")

function calculateChange (_money, _price) {
   let userMoney = Number(MONEY_FIELD.value);
    const MONEY_FIELD = document.getElementById("moneyField");
    if (userMoney >= 4) {
        let calculateChange = _money - _price;
    OUTPUT.innerHTML += "<p>You will get $"+calculateChange+ " change.";
      }
    
return "hello world";
}



